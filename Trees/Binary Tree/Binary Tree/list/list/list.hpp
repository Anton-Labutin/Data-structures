...
#include "../list_node/list_node.hpp"
...
{
    ListNode<T> * newNode = new ListNode<T>(key, nullptr, GetHead());

    if (GetTail() == nullptr) {
        SetTail(newNode);
    } else {
        GetHead() -> SetPrevNode(newNode);
    }
...
List<T>::PushBack(const T& key)
{
    ...
    ListNode<T> * newNode = new ListNode<T>(key, tail, nullptr);
...
List<T>::PopBack() throw(std::runtime_error)
{
    ...
    if (len) {
        ListNode<T> *nodeBeforeTail = GetTail() -> GetPrevNode();
...
List<T>::Erase(const T& key) throw(std::runtime_error)
{
    if (GetLen()) {
        ...
        if (keyNode) {
            ListNode<T> *nodeBeforeKey = keyNode -> GetPrevNode();
            ...
            if (keyNode -> GetNextNode()) {
                keyNode -> GetNextNode() -> SetPrevNode(nodeBeforeKey);
            }
    ...
    if (nodeBefore) {
        if (GetLen()) {
            if (InList(nodeBefore)) {
                ListNode<T> *newNode = new ListNode<T>(key, nodeBefore -> GetPrevNode(), nodeBefore);
                nodeBefore -> SetPrevNode(newNode);

                if (newNode -> GetPrevNode()) {
...
List<T>::AddAfter(ListNode<T> * const nodeAfter, const T& key) throw(std::runtime_error)
{
    if (nodeAfter) {
        if (GetLen()) {
            if (InList(nodeAfter)) {
                ListNode<T> * newNode = new ListNode<T>(key, nodeAfter, nodeAfter -> GetNextNode());
