...
class List {
    ...
    const ListNode<T>* const
    GetLastNode() const;

    ListNode<T>*
    GetNodeBeforeLast() throw(std::runtime_error);
...
template <typename T>
ListNode<T>*
List<T>::GetNodeBeforeLast() throw(std::runtime_error)
{
    if (GetLen()) {
        return GetNodeBefore(GetLastNode());
    } else {
        throw std::runtime_error("GetNodeBeforeLast : list is empty");
    }
}
...
{
    const ListNode<T> * const lastNode = GetLastNode();
...
List<T>::PopBack() throw(std::runtime_error)
{
    ...
    if (len) {
        if (len > 1) {
            ListNode<T> *nodeBeforeLast = GetNodeBeforeLast();
            delete nodeBeforeLast -> GetNextNode();
            nodeBeforeLast -> SetNextNode(nullptr);
        } else {
            delete GetHead();
            SetHead(nullptr);
        }
